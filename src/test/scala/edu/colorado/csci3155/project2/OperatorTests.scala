...
class OperatorTests extends AnyFunSuite {
    ...
    test("Place Right Operator Test") {
        val s =
            """
              |let x = 10 + 15 in
              | let y = rectangle(x) +|+ circle(x) in
        ...
        v1 match {
            case FigValue(canvas) => {
                ...
                assert(listOfObjects.forall({
                    case MyCircle(c, r) => {
                        assert (math.abs(c._1 - 50.0)<= 1E-6 && math.abs(c._2-25) <= 1E-6, s"Center of circle wrong: expected (50,25), got $c")
                        assert ( math.abs(r - 25.0) <= 1E-6, "Radius of circle wrong: expected 25, got $r")
                        true
                    }
                    case Polygon(lst)=> {
    ...
    test("Place Top Test") {
        ...
        v1 match {
            case FigValue(canvas) => {
                ...
                assert(listOfObjects.forall({
                    case MyCircle(c, r) => {
                        assert (math.abs(c._1 -25 )<= 1E-6 && math.abs(c._2 -50) <= 1E-6, s"Center of circle wrong: expected (25,50), got $c")
                        assert ( math.abs(r - 25.0) <= 1E-6, "Radius of circle wrong: expected 25, got $r")
                        true
                    }
                    case Polygon(lst)=> {
                        val expectedCoords: List[(Double, Double)] = List((0,0), (25.0,0), (25, 25), (0, 25))
                        assert (lst.length == 4, "Rectangle must have 4 coordinates")
                        lst.forall( c => expectedCoords.count(p => math.abs(p._1 - c._1) <= 1E-06
                          && math.abs(p._2 - c._2) <= 1E-06) == 1
                        )
