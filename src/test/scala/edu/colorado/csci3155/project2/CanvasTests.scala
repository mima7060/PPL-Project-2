...
class CanvasTests extends AnyFunSuite {
    ...
    test("test scale function"){
        /* We are not writing test cases for the scale 
          ...
          on your own? */
        val scalar = 2.0
        val circ = new MyCircle((2.0,2.0), 1.0)
        val poly = new Polygon( List( (1.0,1.0), (1.0,2.0), (2.0,1.0), (2.0,2.0) ) )
        val canv = new MyCanvas( List(circ, poly) )
        val scaledCirc = new MyCircle((4.0,4.0), 2.0)
        val scaledPoly = new Polygon( List( (2.0,2.0), (2.0,4.0), (4.0,2.0), (4.0,4.0) ) )
        val testCanv = new MyCanvas( List(scaledCirc, scaledPoly) )
        val myCirc = canv.scale(scalar).getListOfObjects.head.asInstanceOf[MyCircle]
        val myPoly = canv.scale(scalar).getListOfObjects.tail.head.asInstanceOf[Polygon]
        val testCirc = testCanv.getListOfObjects.head.asInstanceOf[MyCircle]
        val testPoly = testCanv.getListOfObjects.tail.head.asInstanceOf[Polygon]
        assert(myCirc.c == testCirc.c && myCirc.r == testCirc.r)
        assert(myPoly.cList == testPoly.cList)
    }
}
